$(document).ready(function() {
	...
	//寻求合作end
	//设置左侧导航
	setLeftNav('#companyMyCollectionInvestor');
	commonInit();
			...
			var li = '<div id="shaDowShow'+i+'"  class="mydiv1" onmouseout="delShaDowClass('+i+')" onmouseover="addShaDowClass('+i+')"><li>'
				...
				li+="<div class='cs'>"	 
					li+="<div class='mod borderright'>" 
						li+='<a href="'+moreInfoUrl+'"><div class="cs_tis">'+"投资领域："+FrontCommonFunction.setInvestorDomain(queryReturnList[i].investorUserResultModel.investDomain)+"</div></a>"
						li+='<a href="'+moreInfoUrl+'"><div class="cs_tis">'+"投资阶段："+FrontCommonFunction.setInvestorPhase(queryReturnList[i].investorUserResultModel.investPhase)+"</div></a>"
						li+="<div class=''><a class='fl rems'></a>"+'<a href="'+moreInfoUrl+'">'+"<span class='fl'>"+"投资概述&nbsp;:&nbsp;"+FrontCommonFunction.getResultMaitText(queryReturnList[i].investorUserResultModel.investOutline,60,"#investorUserMoreInfo",queryReturnList[i].investorUserResultModel.id)+"</span></a><div class='clear'></div></div>"
					li+="</div>"
				...
				li+="</div>"	 
				li+="<div class='f'>"
				li+='<a href="'+moreInfoUrl+'"><div style=" margin-top:60px" class="fl">'+FrontCommonFunction.replaceNull(queryReturnList[i].investorUserResultModel.province)+"</div></a>" 
				li+='<div class="fr" style=" margin-top:38px">'	
...
function getData() {
	var paramTemp = {};
	var investorQueryModel={};
	var investorUserQueryModel = {};
	investorQueryModel['investDomain']=$.trim($("#domainId").val());
	investorQueryModel['investAmount']=$.trim($('#amountId').val());
	investorQueryModel['investPhase']=$.trim($('#investPhaseId').val());
	 if ($("#onlySeeLocalCity").hasClass("on")) {
		// 加入勾选本市值
