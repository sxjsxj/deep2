...
function initOrganizationUserManager() {
	//下拉注册事件
	setSelectValue();
...
//	});
	...
	$("#checkBoxInvestType [id^=investTypeCheckBox]").click(function() {
		...
		if(text!==""){
			...
			$("#checkResult2").html('<font color="red">至少选择一种投资方式</font>');
		}
	...
	$('#saveBtn').click(function() {
		...
		var name=$("#name").val();
		if(name===""){
			$("#nameCheckResult").html('<font color="red">必填</font>');
		}
		...
		if(name!=="" && imgCheckFlag ==="" && fileCheckFlag===""
			&& province!==""&& city!==""&& county!=="" && scale!=="" && investType!==""
...
function save() {
	...
	var url = "";
	var commonInvestorUserId = $("#commonInvestorUserId").val();
	if (commonInvestorUserId !== "" && commonInvestorUserId !== undefined
			&& commonInvestorUserId !== "undefined") {
	...
	$.ajaxFileUpload({
		...
		type : 'post',
		formId : 'investorOrganizationForm',
		timeout : 100000,
